...
const tool = { type: 'function', function: {
  ...
  parameters: { type: 'object', properties: {
    send: { type: 'boolean' }, text: { type: 'string', get description(){return prompt('proactive.text');} }, image:{type:'boolean',get description(){return prompt('proactive.image');}},expectsReply:{type:'boolean',get description(){return prompt('proactive.expectsReply');}},twitterPost:{type:['string','null'],maxLength:280,get description(){return prompt('proactive.twitterPost');}},
  }, required: ['send', 'text', 'image', 'expectsReply', 'twitterPost'], additionalProperties: false },
} };

export async function generateProactiveLine(s: GameState, c: Content, signal?: AbortSignal, deliver?: (message: {id:string;text:string;expectsReply:boolean;twitterPost?:string;image?:string;imageCaption?:string}) => Promise<void>) {
  ...
  const results = await Promise.allSettled(selected.map(async character => {
    ...
    const lineTool=socialAppEnabled(s,'twitter')?tool:{...tool,function:{...tool.function,parameters:{...tool.function.parameters,properties:Object.fromEntries(Object.entries(tool.function.parameters.properties).filter(([key])=>key!=='twitterPost')),required:tool.function.parameters.required.filter(key=>key!=='twitterPost')}}};
    registry.register({schema:lineTool,parse:raw=>{const value=JSON.parse(raw),twitterPost=socialAppEnabled(s,'twitter')&&typeof value.twitterPost==='string'&&value.twitterPost.trim()?value.twitterPost.trim():undefined;if(typeof value.send!=='boolean'||typeof value.text!=='string'||typeof value.image!=='boolean'||value.text.length>1500||(twitterPost?.length??0)>280)throw Error('主動 LINE 工具參數無效。');return {...value,twitterPost,expectsReply:value.expectsReply===true} as {send:boolean;text:string;image:boolean;expectsReply:boolean;twitterPost?:string};},execute:async value=>value.send&&value.text.trim()||value.twitterPost?{id:character.id,text:value.send?value.text.trim():'',image:value.send&&value.image,expectsReply:value.send&&value.expectsReply,twitterPost:value.twitterPost}:null,validateResult:value=>{if(value&&!value.text&&!value.twitterPost)throw Error('主動 LINE 工具結果無效。');}});
    const response = await modelFetch(endpoint, {
      ...
      body: JSON.stringify({ model: settings.model, temperature: 0.8, max_tokens: 1200,
        ...
        messages: [{ role: 'system', content: [configuredPrompt(c.settings.systemPrompt),configuredPrompt(character.prompt,{name:character.name,bio:character.bio}),knowledge,webKnowledge,
          ...
          prompt('proactive.context',{profile:JSON.stringify(currentProfile(character,s,c)),affection:s.affection[character.id]??0,route:s.route===character.id}),
    ...
    const message=await registry.execute(call?.name??'',call?.arguments??'{}') as {id:string;text:string;image:boolean;expectsReply:boolean;twitterPost?:string}|null;
    ...
    const delivered={id:message.id,text:message.text,expectsReply:message.expectsReply,twitterPost:message.twitterPost,...(generated.url?{image:generated.url,imageCaption:generated.caption}:{})};
...
async function generateContactGreeting(s:GameState,c:Content,id:string,signal?:AbortSignal){
 ...
 const schema={type:'function',function:{name:'send_contact_greeting',description:prompt('proactive.contactTool'),parameters:{type:'object',properties:{text:{type:'string',description:prompt('proactive.text')}},required:['text'],additionalProperties:false}}};
 ...
 const value=JSON.parse(call.function.arguments);if(typeof value.text!=='string'||!value.text.trim()||value.text.length>1500)return;
 return value.text.trim() as string;
}
...
export function startContactGreeting(owner:string,s:GameState,c:Content,id:string){
 ...
 void generateContactGreeting(snapshot,c,id).then(async text=>{
  if(!text||!await committed)return;
  ...
  const next=structuredClone(current);next.messages[id]=[...(next.messages[id]??[]),{id:crypto.randomUUID(),phase:snapshot.phase,from:id,text,date:snapshot.date}].slice(-100);
...
export function startProactiveLine(owner: string, s: GameState, c: Content, old?: GameState) {
  ...
  void generateProactiveLine(snapshot, c, undefined, async ({id, text, image, imageCaption,expectsReply,twitterPost}) => {
    ...
    if(twitterPost&&socialAppEnabled(current,'twitter'))publishCharacterTwitterPost(next,c,id,twitterPost);
    await commitConcurrent(owner, next, current,true);
