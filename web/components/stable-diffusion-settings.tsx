...
export function StableDiffusionSettings({registerSave,onStatusChange}:Props){
 ...
 <label>{uiText("model_family")}<select value={settings.modelFamily} onChange={e=>{const modelFamily=e.target.value as ModelFamily;setSettings({...settings,...modelDefaults(modelFamily),modelFamily,checkpoint:''});}}><option value="Illustrious">{uiText("technical.illustrious")}</option><option value="Pony">{uiText("technical.pony")}</option></select></label>
