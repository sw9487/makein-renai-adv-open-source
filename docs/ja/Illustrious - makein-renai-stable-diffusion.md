**言語:** [繁體中文](../zh-TW/Illustrious - makein-renai-stable-diffusion.md) · [English](../en/Illustrious - makein-renai-stable-diffusion.md) · [日本語](Illustrious - makein-renai-stable-diffusion.md)
...
温水 佳樹：
私服間で干渉する場合は、ネガティブプロンプト等で調整してください。

※2025.03.02

IllustriousXL1.0検証用にデータセットを加えて学習していましたが、よくよく考えたらデータセットを加えたら検証にならないのでv0.1でも普通に学習させました。

データセット追加ついでにノイズ除去と画質調整処理を行っているので、最初にアップロードしたLoraよりも使いやすくなっているはずです。

需要があるかわかりませんが、IllustriousXL1.0検証用のLoraも置いておきます。v1.1のLoraと同じデータセット同じ設定同じepoc数です。
...
かなりのDL数があり需要が大きいLoraだったので、ノイズ除去･画質調整･キャプション再編集を行って再学習したLora配布します。

衣装再現や生成画像の画質が向上しています。
...
肌が白くなってしまう場合は、tanやdark skin等を併用してください。
...
トリガー: Nukumizu Kazuhiko, short hair, black hair, brown eyes, ahoge
...
水着: swimshorts (+blue hoodie)

体操服: gymwear

必要に応じて green necktie などの修飾語を追加すると、より正確に fit できます。

女性化を防ぐには、1boy の重みを上げて male を追加し、ネガティブに shota などを追加してください。
